...
bool IsPrimeDivByPrime(mpz_t x)
{
    ...
    for(unsigned long long i = 0; i<PrimesCount32; i++)
    {
        ...
        mpz_mod_ui(tmp,x,Prime);
        if(mpz_cmp_ui(tmp,0)==0) return false;
    }
...
{
    unsigned long long MaxToTest = sqrtl(x);
    uint32_t Prime=0;
    if (x<=1) return false;
    for (unsigned long long i = 0; i<PrimesCount32; i++)
    {
...
{
    unsigned long MaxToTest = sqrtl(x);
    //cout << x << endl;
    if (x <= 30)
    {
        if (x == 2)
